...
import { restoreTodoAction } from "@/app/actions/todos";
...
function ArchivedTodoRow({ groupName, item }: { groupName: string; item: TodoDTO }) {
  const [state, formAction] = useActionState(restoreTodoAction, emptyActionState);
  ...
  return (
    <li className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-black/10 bg-white px-4 py-3 dark:border-white/10 dark:bg-white/5">
      <div className="min-w-0">
        <span className="truncate font-medium line-through opacity-70">
          {item.title}
        </span>
        <p className="muted mt-0.5">
          Added by {item.isOwn ? "you" : item.memberName}
          {item.completedAt ? ` · completed ${formatDateTime(new Date(item.completedAt))}` : ""}
        </p>
      </div>
...
export function ArchivedTodoList({
  ...
  if (items.length === 0) {
    return <p className="muted">No completed items yet.</p>;
  }

  return (
    <ul className="space-y-2">
      {items.map((item) => (
        <ArchivedTodoRow key={item.id} groupName={groupName} item={item} />
      ))}
    </ul>
  );
}
